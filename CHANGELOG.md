...
## [Unreleased]
- **Domande PDF 04 — opzioni accorciate (limite 130 caratteri):** Riscritte tutte le 205 opzioni delle 41 domande del PDF 04 per rientrare entro 130 caratteri (lunghezza media scesa da 174 a 89, max 126), eliminando le "opzioni-paragrafo". Mantenuti invariati domande, spiegazioni, posizione della risposta corretta (`r`) e tutti i micro-errori dei distrattori. Aggiornato il prompt di generazione con il limite rigido di 130 caratteri per opzione. Bump cache → `bioquiz-v26`.
- **Audit domande PDF 04 — fix ambiguità "doppia risposta corretta":** In Q15 (catabolismo adrenalina) e Q34 (sintesi cortisolo) il distrattore `opt0` era scientificamente corretto quanto la risposta giusta (differiva solo per informazioni aggiuntive): due risposte valide. Introdotto un micro-errore netto in entrambi i distrattori — Q15: "acido omovanillico" (catabolita della dopamina) al posto di "vanilmandelico" (corretto per l'adrenalina); Q34: "17β-idrossilasi" al posto di "17α-idrossilasi" nella prima tappa. Bump cache → `bioquiz-v25`.
...
- **Service Worker:** Aggiornata la cache (`v18`) per forzare tutti i dispositivi a scaricare immediatamente il fix della libreria chimica senza svuotare la cache manuale.
